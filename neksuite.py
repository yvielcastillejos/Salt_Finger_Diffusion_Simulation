...
def readnek(fname):
	...
	for v in vars:
		if (v == 'X'):
			...
			var[5] = 1 # TODO: need to know how this works    
	#
	...
	#
    # read scalar2 fields
	data.lims.scal2[:,0] =  float('inf')
	data.lims.scal2[:,1] = -float('inf')
	for iel in elmap:
		for ivar in range(var[5]): # if var[5] == 0, scalars are not read
			fi = infile.read(npel*wdsz)
			fi = list(struct.unpack(emode+npel*realtype, fi))
			ip = 0
			for iz in range(lr1[2]):
				for iy in range(lr1[1]):
					#data.elem[iel-1].scal[ivar,iz,iy,:] = fi[ip:ip+lr1[0]]
					data.elems.scal2[ivar,iel-1,iz,iy,:] = fi[ip:ip+lr1[0]]                    
					ip += lr1[0]
			data.lims.scal2[ivar,0] = min([data.lims.scal2[ivar,0]]+fi)
			data.lims.scal2[ivar,1] = max([data.lims.scal2[ivar,1]]+fi)
